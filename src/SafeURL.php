<?php
namespace fin1te\SafeURL;

use fin1te\SafeURL\Exception;
use fin1te\SafeURL\Exception\InvalidURLException;
use fin1te\SafeURL\Exception\InvalidURLException\InvalidDomainException;
use fin1te\SafeURL\Exception\InvalidURLException\InvalidIPException;
use fin1te\SafeURL\Exception\InvalidURLException\InvalidPortException;
use fin1te\SafeURL\Exception\InvalidURLException\InvalidSchemeException;
...
class SafeURL {
    ...
    /**
     * Returns new instance of SafeURL\SafeURL
     *
     * @param $curlHandle resource         A valid cURL handle
     * @param $options    SafeURL\Options optional
     */
    ...
    public function setCurlHandle($curlHandle) {
         if (!is_resource($curlHandle) || get_resource_type($curlHandle) != 'curl') {
            ...
            throw new Exception("SafeURL expects a valid cURL resource - '" . gettype($curlHandle) . "' provided.");
        }
    ...
    public static function execute($url, $curlHandle = null, Options $options = null) {
        ...
        if (isset($this) && get_class($this) == __CLASS__) {
            ...
            $safeURL = new SafeURL($curlHandle, $options);
        }
        ...
        $redirected     = false;
        $redirectCount  = 0;
        $redirectLimit  = $safeURL->getOptions()->getFollowLocationLimit();
        $followLocation = $safeURL->getOptions()->getFollowLocation();
        do {
            //Validate the URL
            $url = Url::validateUrl($url, $safeURL->getOptions());

            //Are there credentials, but we don't want to send them?
            if (!$safeURL->getOptions()->getSendCredentials() &&
            ...
            if ($safeURL->getOptions()->getPinDns()) {
                //Send a Host header
